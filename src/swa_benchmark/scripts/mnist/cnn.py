...
if __name__ == "__main__":
    ...
    model = CNN(input_size=1, hidden_size=64, output_size=10, n_layers=3).to(device)
    ...
    pretrained_model = CNN(input_size=1, hidden_size=64, output_size=10, n_layers=3).to(device)
    pretrained_model.load_state_dict(model.state_dict())

    swa_model = CNN(input_size=1, hidden_size=64, output_size=10, n_layers=3).to(device)
    ...
    plot_loss_landspace(device, pretrained_model, model, swa_model, criterion, test_dl, train_dl, point_names=["Pretrained", "Model", "SWA"], n_points=5)
    plt.show()
