...
def train(train_dl, test_dl, optimizer, scheduler, criterion, model, epochs, device, metric=None):
...
def swa_train(train_dl, test_dl, optimizer, scheduler, criterion, model, epochs, swa_model, swa_length, device, metric=None):
    swa_n = 0
    for epoch in range(epochs):
        start = time.time()
        train_loss = train_epoch(epoch, train_dl, optimizer, scheduler, criterion, model, device)
