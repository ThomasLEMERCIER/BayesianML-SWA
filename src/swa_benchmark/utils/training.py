...
def train(
    ...
    for epoch in range(epochs):
        ...
        print(
            f"Epoch {epoch+1}/{epochs}, Train loss: {train_loss:.4f}, Test loss: {test_loss:.4f}{', Test metric: ' + format(test_metric, '.4f') if metric is not None else ''}, Time: {time.time() - start:.2f} s"
        )
...
def swa_train(
    ...
    for epoch in range(epochs):
        ...
        if epoch > 0 and (epoch + 1) % swa_length == 0:
            ...
            print(
                f"SWA model test loss: {swa_loss:.4f}{', SWA test metric: ' + format(swa_metric, '.4f') if metric is not None else ''}, Learning rate: {optimizer.param_groups[0]['lr'].item():.4f}"
            )
            ...
            if return_ensemble:
                ...
                print(
                    f"Ensemble model test loss: {ensemble_loss:.4f}{', Ensemble test metric: ' + format(ensemble_metric, '.4f') if metric is not None else ''}"
                )
